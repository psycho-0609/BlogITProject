$(document).ready(function () {
    ...
    function getData() {
        $.ajax({
            url: "/api/admin/account/getAll" + keySearch,
            method: "get",
            dataType: 'json'
        }).done(function (res) {
            fetchData(res)
    ...
    function fetchData(data) {
        ...
        if (data.accounts.length > 0) {
            data.accounts.forEach(el => res += write(el))
        } else {
            res = " <td colspan='5'>\n" +
    ...
    function write(el) {
        ...
        let item = " <tr>\n" +
            "                                    <td>" + el.id + "</td>\n" +
            "                                    <td><a href='/author/" + el.userDetail.id + "+page=1'>" + el.userDetail.firstName + el.userDetail.lastName + "</a></td>\n" +
            "                                    <td>" + el.email + "</td>\n" +
            "                                    <td>" + status + "</td>\n" +
